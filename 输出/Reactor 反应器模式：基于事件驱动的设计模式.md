...
Reactor 模式**适用于** IO 密集型应用，如网络服务器、消息队列、代理服务器；在这种场景下，CPU 大部分时间是在等待网络数据，使用 Reactor 模式可以最大化的利用资源；

Reactor 模式**适用于**高并发连接场景，在这种场景下，如果使用传统的“一个连接，一个线程”的模式，系统资源会很快被耗尽，Reactor 模式使用少量的 IO 线程便可以管理大量的连接；

Reactor 模式**不适用于**计算密集型应用，由于 IO 线程是有限的，当所有 IO 线程都被占用来执行任务，后续所有的事件都会被阻塞。
