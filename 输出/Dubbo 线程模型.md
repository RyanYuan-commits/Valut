...
在基于 Reactor 模式实现的网络通信应用中，负责连接、事件监听和处理的 I/O 线程是非常珍贵的资源：它们直接负责连接建立和数据读写，如果 I/O 线程资源耗尽，会导致整个系统陷入瘫痪；因此，在 I/O 线程上不应该处理耗时过久的任务[^1]；

为此，Dubbo 设计了业务线程池，用来处理耗时任务，并提供了分发器来处理任务从 I/O 线程到业务线程的分发。
